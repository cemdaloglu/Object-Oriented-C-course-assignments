...
public:
    ...
    decltype(auto) operator+=(const Matrix<numRows, numCols, T>& b);
    // output
    void print() const;
...
template<std::size_t numRows, std::size_t numCols, typename T>
auto Matrix<numRows, numCols, T>::operator()(int i, int j)
{
    if (i < 0 || i >= numRows)
    {
...
template<std::size_t numRows, std::size_t numCols, typename T>
auto Matrix<numRows, numCols, T>::operator()(int i, int j) const
{
    if ( i < 0 || i >= numRows)
    {
        std::cerr << "Illegal row index " << i;
...
template<std::size_t numRows, std::size_t numCols, typename T>
auto Matrix<numRows, numCols, T>::operator[](int i)
{
    if (i < 0 || i >= numRows)
    {
        std::cerr << "Illegal row index " << i;
...
template<std::size_t numRows, std::size_t numCols, typename T>
const auto Matrix<numRows, numCols, T>::operator[](int i) const
{
    if (i < 0 || i >= numRows)
    {
        std::cerr << "Illegal row index " << i;
...
template<std::size_t numRows, std::size_t numCols, typename T>
decltype(auto) Matrix<numRows, numCols, T>::operator*=(T x)
{
...
template<std::size_t numRows, std::size_t numCols, typename T>
decltype(auto) Matrix<numRows, numCols, T>::operator+=(const Matrix<numRows, numCols, T>& x)
{
    for (int i = 0; i < numRows; ++i)
        for (int j = 0;j < numCols; ++j)
            entries[i][j] += x[i][j];
